...
2. **Интеграция CMake:** Отдельный мастер для создания проектов на базе современного сборочного конвейера CMake с генерацией полноценных `CMakeLists.txt` и файла тулчейна.
...
6. **Диаграмма использования памяти (Memory Usage Report):** Автоматический вывод подробного текстового отчета по занятой памяти (RAM, Flash/SPIFI, EEPROM) с графической индикацией заполнения в консоли Eclipse после каждой успешной сборки.
...
<p align="center">
  <b>Диаграмма использования памяти в консоли сборщика</b><br/>
  <img src="images/interface3.png" width="550" style="margin-top: 10px; border-radius: 4px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);"/>
</p>
...
## 🛠 Полный CMakeLists.txt для проектов MIK32

При создании проекта с типом сборки **CMake**, плагин автоматически генерирует в корне проекта полноценный файл `CMakeLists.txt`
